    ...
    <div class="w-full max-w-4xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden mt-6">
      <div class="p-6">
        ...
        <div class="flex items-center justify-between mb-4">
          <p class="text-xl font-semibold text-green-600">Salario: ${{ formatCurrency(vacancy.salary) }}</p>
          <p class="text-gray-600">Empresa: {{ vacancy.company_name }} (ID: {{ vacancy.company_id }})</p>
        </div>
        <div class="flex flex-wrap mb-6">
          <p class="bg-gray-200 text-gray-800 px-4 py-2 rounded-full mr-2 mb-2">Área: {{ vacancy.area ? vacancy.area.area_name : 'No disponible' }}</p>
          <p class="bg-gray-200 text-gray-800 px-4 py-2 rounded-full mr-2 mb-2">Puesto: {{ vacancy.position ? vacancy.position.position_name : 'No disponible' }}</p>
          <p class="bg-gray-200 text-gray-800 px-4 py-2 rounded-full mb-2">Provincia: {{ vacancy.province ? vacancy.province.province_name : 'No disponible' }}</p>
        </div>
